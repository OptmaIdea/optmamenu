-- POS_9 / v0.9.14 — RPC de estorno de transferência recebida — V2
-- Objetivo:
-- - Recriar reverse_received_stock_transfer com criação segura do saldo da origem.
-- - Evitar INSERT duplicado em inventory_location_balances quando não houver constraint única.
--
-- Uso: rode este arquivo no SQL Editor do Supabase no lugar da versão anterior.
...
BEGIN
  ...
  LOOP
    ...
    WHERE store_id = v_transfer.store_id
      ...
      AND COALESCE(variant_id, v_zero_variant) = COALESCE(v_item.variant_id, v_zero_variant)
    FOR UPDATE;

    IF COALESCE(v_destination_previous, 0) < v_item.received_qty THEN
      RAISE EXCEPTION 'Não há saldo suficiente no destino para estornar a transferência. Produto: %, saldo atual: %, necessário: %.',
        v_item.product_id,
        COALESCE(v_destination_previous, 0),
        v_item.received_qty;
    END IF;
