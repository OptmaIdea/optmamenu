-- POS_9 / v0.9.14 — RPC de estorno de transferência recebida
-- Objetivo:
-- - Permitir estornar uma transferência já recebida sem apagar histórico.
-- - Retirar do destino a quantidade recebida e devolver para a origem.
-- - Registrar movimentos inversos em stock_movements.
--
-- Uso: rode no SQL Editor do Supabase.
-- Depois integrar no frontend via stockService.reverseReceivedStockTransfer.
...
BEGIN
  ...
  IF NOT public.is_store_member(v_transfer.store_id, v_user_id) THEN
  ...
  -- Trava os itens antes de validar saldos.
  PERFORM 1
  FROM public.stock_transfer_items
  WHERE transfer_id = p_transfer_id
  FOR UPDATE;

  -- Valida se o destino ainda tem saldo suficiente para devolver tudo que foi recebido.
  FOR v_item IN
    SELECT
      id,
      product_id,
  ...
  -- Aplica os movimentos inversos: sai do destino e volta para origem.
  FOR v_item IN
    SELECT
      id,
      product_id,
    ...
    ORDER BY created_at, id
  LOOP
    -- Destino: baixa a quantidade recebida anteriormente.
    SELECT COALESCE(on_hand, 0)
    ...
    -- Origem: devolve a quantidade ao local de origem.
