-- POS_9 / v0.9.14 — RPC de estorno de transferência recebida V4
-- Corrige ambiguidade de nomes em PL/pgSQL qualificando colunas com alias.
-- Rode no SQL Editor do Supabase para substituir a função anterior.
...
BEGIN
  ...
  FOR v_item IN
    ...
    FROM public.stock_transfer_items sti
    WHERE sti.transfer_id = p_transfer_id
      AND sti.received_qty > 0
    ORDER BY sti.created_at, sti.id
  LOOP
    SELECT COALESCE(ilb.on_hand, 0)
    ...
    WHERE ilb.store_id = v_transfer.store_id
      ...
      AND COALESCE(ilb.variant_id, '00000000-0000-0000-0000-000000000000'::uuid) = COALESCE(v_item.variant_id, '00000000-0000-0000-0000-000000000000'::uuid)
    FOR UPDATE;
  ...
  LOOP
    ...
    WHERE NOT EXISTS (
      SELECT 1
      FROM public.inventory_location_balances ilb_origin_exists
      WHERE ilb_origin_exists.store_id = v_transfer.store_id
        AND ilb_origin_exists.location_id = v_transfer.source_location_id
        AND ilb_origin_exists.product_id = v_item.product_id
        AND COALESCE(ilb_origin_exists.variant_id, '00000000-0000-0000-0000-000000000000'::uuid) = COALESCE(v_item.variant_id, '00000000-0000-0000-0000-000000000000'::uuid)
    );

    SELECT COALESCE(ilb_origin.on_hand, 0)
      INTO v_origin_previous
    FROM public.inventory_location_balances ilb_origin
    WHERE ilb_origin.store_id = v_transfer.store_id
      AND ilb_origin.location_id = v_transfer.source_location_id
      AND ilb_origin.product_id = v_item.product_id
      AND COALESCE(ilb_origin.variant_id, '00000000-0000-0000-0000-000000000000'::uuid) = COALESCE(v_item.variant_id, '00000000-0000-0000-0000-000000000000'::uuid)
    FOR UPDATE;

    v_origin_new := COALESCE(v_origin_previous, 0) + v_item.received_qty;

    UPDATE public.inventory_location_balances ilb_origin_update
    SET
      on_hand = v_origin_new,
      updated_at = now()
    WHERE ilb_origin_update.store_id = v_transfer.store_id
      AND ilb_origin_update.location_id = v_transfer.source_location_id
      AND ilb_origin_update.product_id = v_item.product_id
      AND COALESCE(ilb_origin_update.variant_id, '00000000-0000-0000-0000-000000000000'::uuid) = COALESCE(v_item.variant_id, '00000000-0000-0000-0000-000000000000'::uuid);
...
IS 'Estorna transferência recebida: remove do destino, devolve para origem, registra movimentos inversos e preserva histórico.';
