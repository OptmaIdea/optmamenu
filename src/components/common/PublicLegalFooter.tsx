...
export default function PublicLegalFooter() {
    ...
    const candidates: Array<Omit<SocialLink, 'href'> & { href: string | null }> = [
        { key: 'instagram', label: 'Instagram', href: normalizeExternalUrl(storeConfig?.social_links?.instagram), icon: <Instagram className="h-5 w-5" aria-hidden="true" /> },
        { key: 'facebook', label: 'Facebook', href: normalizeExternalUrl(storeConfig?.social_links?.facebook), icon: <Facebook className="h-5 w-5" aria-hidden="true" /> },
        { key: 'tiktok', label: 'TikTok', href: normalizeExternalUrl(storeConfig?.social_links?.tiktok), icon: <Music2 className="h-5 w-5" aria-hidden="true" /> },
        { key: 'twitter', label: 'X / Twitter', href: normalizeExternalUrl(storeConfig?.social_links?.twitter), icon: <Twitter className="h-5 w-5" aria-hidden="true" /> },
        { key: 'website', label: 'Site da loja', href: normalizeExternalUrl(storeConfig?.social_links?.website), icon: <Globe2 className="h-5 w-5" aria-hidden="true" /> },
    ];
        ...
        <footer className={`border-t border-slate-200 bg-slate-100 px-4 pt-7 text-center text-slate-600 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300 ${hasFixedCartBar ? 'pb-36 sm:pb-32' : 'pb-7'}`}>
            <div className="mx-auto max-w-5xl">
                ...
                <p className="mx-auto mt-2 max-w-2xl text-xs leading-5 text-slate-500 dark:text-slate-400 sm:text-sm">
                    {storeSlug ? 'Documentos da loja e preferências de privacidade deste dispositivo.' : 'Documentos da plataforma e preferências de privacidade deste dispositivo.'}
                </p>
                ...
                {socialLinks.length > 0 && (
                    <div aria-label="Redes sociais da loja" className="mt-5 flex items-center justify-center gap-2">
                        {socialLinks.map((social) => (
                            <a key={social.key} href={social.href} target="_blank" rel="noreferrer" aria-label={social.label} title={social.label} className="flex h-10 w-10 items-center justify-center rounded-full text-slate-600 transition hover:bg-white hover:text-slate-950 dark:text-slate-300 dark:hover:bg-slate-900 dark:hover:text-white">
                                {social.icon}
                            </a>
                        ))}
                    </div>
