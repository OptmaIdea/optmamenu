            ...
            <div className="mx-auto grid max-w-5xl gap-6 md:grid-cols-[1fr_auto] md:items-end">
                <div>
                    ...
                    <nav aria-label="Documentos legais" className="mt-4 flex flex-wrap gap-x-5 gap-y-3 text-sm font-bold">
                        <Link to="/terms" className="text-emerald-700 underline-offset-4 hover:underline dark:text-emerald-300">
                            Termos de Uso
                        </Link>
                        <Link to="/politica-privacidade" className="text-emerald-700 underline-offset-4 hover:underline dark:text-emerald-300">
                            Política de Privacidade
                        </Link>
                        <Link to="/politica-cookies" className="text-emerald-700 underline-offset-4 hover:underline dark:text-emerald-300">
                            Política de Cookies
                        </Link>
                        <button
                            type="button"
                            onClick={openCookiePreferences}
                            className="inline-flex items-center gap-1.5 text-emerald-700 underline-offset-4 hover:underline dark:text-emerald-300"
                        >
                ...
                <div className="text-xs leading-5 text-slate-500 dark:text-slate-400 md:text-right">
                    <p>Termos: {TERMS_VERSION}</p>
                    <p>Privacidade: {PRIVACY_VERSION}</p>
                    <p>Cookies: versão {COOKIES_VERSION}</p>
                    <p className="mt-2">
                        <a
                            href="https://optmamenu.optmaidea.com.br"
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center gap-1 font-bold text-slate-700 hover:underline dark:text-slate-200"
                        >
                            OptmaMenu <ExternalLink className="h-3.5 w-3.5" aria-hidden="true" />
                        </a>
                        {' · '}
                        <a
                            ...
                            className="inline-flex items-center gap-1 font-bold text-slate-700 hover:underline dark:text-slate-200"
                        >
                            OptmaIdea <ExternalLink className="h-3.5 w-3.5" aria-hidden="true" />
                        </a>
                    </p>
                </div>
            </div>
