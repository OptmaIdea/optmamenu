...
function paymentMethodLabel(code: string, options: AccountsPayablePaymentOptions) {
  return options.payment_methods.find((item) => item.code === code)?.name || code.replaceAll('_', ' ')
}
...
export default function AccountsPayablePage() {
  ...
  const openPayment = (installment: AccountsPayableInstallment) => {
    const fallbackMethod = installment.payment_method_code || detail?.payable.payment_method_code || paymentOptions.payment_methods[0]?.code || ''
    const preferredByMethod = paymentOptions.payment_methods.find((item) => item.code === fallbackMethod)?.preferred_financial_account_id
    const fallbackAccount = installment.preferred_financial_account_id
      || detail?.payable.preferred_financial_account_id
      || preferredByMethod
      || paymentOptions.financial_accounts.find((item) => item.is_default)?.id
      || paymentOptions.financial_accounts[0]?.id
      || ''
  ...
  const changePaymentMethod = (code: string) => {
    ...
    const preferred = paymentOptions.payment_methods.find((item) => item.code === code)?.preferred_financial_account_id
    if (preferred && paymentOptions.financial_accounts.some((item) => item.id === preferred)) setPaymentAccountId(preferred)
  }
  ...
  return (
    ...
    >
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
              <label className="xl:col-span-2"><FieldLabel>Buscar</FieldLabel><div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} className={`${inputClass} pl-9`} placeholder="Fornecedor, AP-, documento..." /></div></label>
      ...
      {termDraft && <ModalFrame title={termDraft.id ? 'Editar condição de pagamento' : 'Nova condição de pagamento'} subtitle="Configure a agenda comercial usada em compras e cotações." onClose={() => !working && setTermDraft(null)} footer={<div className="flex justify-end gap-2"><button disabled={working} onClick={() => setTermDraft(null)} className="rounded-xl border border-gray-300 px-4 py-2 text-sm font-black">Cancelar</button><button disabled={working} onClick={() => void submitTerm()} className="inline-flex items-center gap-2 rounded-xl bg-[#19A999] px-4 py-2 text-sm font-black text-white"><Save size={16} /> Salvar condição</button></div>}><div className="space-y-4">{termDraft.isSystemPreset && <div className="rounded-xl border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900 dark:border-blue-900 dark:bg-blue-950/30 dark:text-blue-100">Modelo inicial do sistema; as alterações valem somente para a loja atual.</div>}<div className="grid gap-4 sm:grid-cols-2"><label><FieldLabel>Nome</FieldLabel><input value={termDraft.name} onChange={(event) => setTermDraft({ ...termDraft, name: event.target.value })} className={inputClass} /></label><label><FieldLabel>Código</FieldLabel><input value={termDraft.code} onChange={(event) => setTermDraft({ ...termDraft, code: event.target.value })} className={inputClass} placeholder="Gerado se ficar vazio" /></label><label><FieldLabel>Modalidade</FieldLabel><select value={termDraft.paymentMode} onChange={(event) => { const paymentMode = event.target.value as 'cash' | 'term'; setTermDraft({ ...termDraft, paymentMode, offsets: paymentMode === 'cash' ? '0' : termDraft.offsets === '0' ? '30' : termDraft.offsets }) }} className={inputClass}><option value="cash">À vista</option><option value="term">A prazo</option></select></label><label><FieldLabel>Dias dos vencimentos</FieldLabel><input disabled={termDraft.paymentMode === 'cash'} value={termDraft.paymentMode === 'cash' ? '0' : termDraft.offsets} onChange={(event) => setTermDraft({ ...termDraft, offsets: event.target.value })} className={inputClass} placeholder="30/60/90" /></label><label className="sm:col-span-2"><FieldLabel>Forma de pagamento prevista</FieldLabel><select value={termDraft.paymentMethodCode} onChange={(event) => setTermDraft({ ...termDraft, paymentMethodCode: event.target.value })} className={inputClass}><option value="">Definir na negociação/baixa</option>{paymentOptions.payment_methods.map((method) => <option key={method.code} value={method.code}>{method.name}</option>)}</select></label></div><div className="grid gap-3 sm:grid-cols-2"><label className="flex items-center justify-between rounded-xl border border-gray-200 p-3 dark:border-gray-700"><div><p className="font-black">Ativa</p><p className="text-xs text-gray-500">Disponível para novas compras.</p></div><input type="checkbox" checked={termDraft.active} onChange={(event) => setTermDraft({ ...termDraft, active: event.target.checked })} className="h-5 w-5 accent-[#19A999]" /></label><label className="flex items-center justify-between rounded-xl border border-gray-200 p-3 dark:border-gray-700"><div><p className="font-black">Padrão da loja</p><p className="text-xs text-gray-500">Fallback sem histórico.</p></div><input type="checkbox" checked={termDraft.isDefault} onChange={(event) => setTermDraft({ ...termDraft, isDefault: event.target.checked, active: event.target.checked ? true : termDraft.active })} className="h-5 w-5 accent-[#19A999]" /></label></div></div></ModalFrame>}
    </PageContainer>
  )
}
