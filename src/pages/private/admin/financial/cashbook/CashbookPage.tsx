...
export default function CashbookPage() {
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            const [entriesData, summaryData, accountsResult] = await Promise.all([
        ...
        >
            ...
            {viewMode === 'libro' ? (
                <div className="grid max-w-full grid-cols-2 gap-2 sm:grid-cols-4 sm:gap-4">
                    ...
                    <div className="rounded-2xl border border-gray-100 bg-white p-3 shadow-sm dark:border-gray-700 dark:bg-gray-800 sm:rounded-3xl sm:p-6"><div className="mb-3 flex items-center gap-3 text-[#19A999]"><div className="rounded-xl bg-[#19A999]/10 p-2"><Wallet size={20} /></div><span className="text-xs font-black uppercase tracking-widest">Resultado</span></div><div className={`text-2xl font-black ${(summary?.balance || 0) >= 0 ? 'text-gray-900 dark:text-white' : 'text-rose-600'}`}>{formatCurrencyPtBr(summary?.balance || 0)}</div><p className="mt-1 text-[10px] font-bold uppercase tracking-tighter text-gray-400">Entradas - saídas do período</p></div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
                        ...
                        <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900"><p className="text-xs font-black uppercase tracking-widest text-gray-400">Resultado</p><p className={`mt-2 text-2xl font-black ${(statement?.period_net || 0) >= 0 ? 'text-[#19A999]' : 'text-rose-600'}`}>{formatCurrencyPtBr(statement?.period_net || 0)}</p><p className="mt-1 text-[10px] font-bold uppercase text-gray-400">Entradas - saídas</p></div>
                        <div className="rounded-2xl border border-teal-100 bg-teal-50 p-4 shadow-sm dark:border-teal-900/60 dark:bg-teal-950/20"><p className="text-xs font-black uppercase tracking-widest text-teal-700 dark:text-teal-300">Saldo final</p><p className={`mt-2 text-2xl font-black ${(statement?.final_balance || 0) >= 0 ? 'text-gray-900 dark:text-white' : 'text-rose-600'}`}>{formatCurrencyPtBr(statement?.final_balance || 0)}</p><p className="mt-1 text-[10px] font-bold uppercase text-gray-500">{statementScopeLabel}</p></div>
                    </div>
                </div>
            )}
            ...
            <div className="max-w-full overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                <div className="flex flex-col gap-3 border-b border-gray-100 p-4 dark:border-gray-700 sm:flex-row sm:items-center sm:justify-between sm:p-6"><div className="flex items-center gap-2"><History size={18} className="text-gray-400" /><h2 className="font-black uppercase tracking-tight text-gray-900 dark:text-white">{viewMode === 'extrato' ? 'Extrato com saldo acumulado' : 'Últimos lançamentos'}</h2></div><div className="flex items-center gap-1 rounded-xl bg-gray-100 p-1 dark:bg-gray-900/60"><button type="button" onClick={() => setViewMode('libro')} className={`rounded-lg px-3 py-1.5 text-xs font-bold uppercase tracking-wider transition-all ${viewMode === 'libro' ? 'bg-white text-gray-900 shadow-xs dark:bg-gray-800 dark:text-white' : 'text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white'}`}>Modo Livro</button><button type="button" onClick={() => setViewMode('extrato')} className={`rounded-lg px-3 py-1.5 text-xs font-bold uppercase tracking-wider transition-all ${viewMode === 'extrato' ? 'bg-white text-gray-900 shadow-xs dark:bg-gray-800 dark:text-white' : 'text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white'}`}>Modo Extrato</button></div></div>
                <div className="border-b border-gray-100 dark:border-gray-700"><div className="flex p-3 md:hidden"><button type="button" onClick={() => setFiltersOpen((open) => !open)} className="inline-flex w-full items-center justify-between rounded-xl border border-gray-200 px-3 py-2 text-sm font-bold text-gray-700 dark:border-gray-700 dark:text-gray-200"><span className="inline-flex items-center gap-2"><SlidersHorizontal size={16} /> Filtros e período</span>{filtersOpen ? <span>Ocultar</span> : <span>Mostrar</span>}</button></div><div className={`${filtersOpen ? 'grid' : 'hidden'} grid-cols-1 gap-3 p-3 sm:p-4 md:grid ${viewMode === 'extrato' ? 'md:grid-cols-6' : 'md:grid-cols-5'}`}><DateRangeFilter periodFilter={periodFilter} onPeriodChange={setPeriodFilter} startDate={startDate} onStartDateChange={setStartDate} endDate={endDate} onEndDateChange={setEndDate} className="col-span-1 md:col-span-3" />{viewMode === 'extrato' ? <label className="space-y-1 md:col-span-2"><span className="text-[10px] font-black uppercase tracking-widest text-gray-400">Conta</span><select value={statementAccountId} onChange={(event) => setStatementAccountId(event.target.value)} className="w-full rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm font-bold text-gray-700 outline-none transition focus:border-[#19A999] dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200"><option value="all">Todas as contas — consolidado</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.name}{account.active ? '' : ' (inativa)'}</option>)}</select></label> : <label className="space-y-1"><span className="text-[10px] font-black uppercase tracking-widest text-gray-400">Cliente em venda</span><div className="relative"><Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" /><input value={customerFilter} onChange={(event) => setCustomerFilter(event.target.value)} placeholder="Nome, descrição, pedido ou cliente" className="w-full rounded-xl border border-gray-200 bg-white py-2 pl-9 pr-3 text-sm font-bold text-gray-700 outline-none transition focus:border-[#19A999] dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200" /></div></label>}<label className="space-y-1"><span className="text-[10px] font-black uppercase tracking-widest text-gray-400">Status</span><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as CashbookStatusFilter)} className="w-full rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm font-bold text-gray-700 outline-none transition focus:border-[#19A999] dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200" disabled={viewMode === 'extrato'}><option value="active">Ativos</option><option value="cancelled">Cancelados</option><option value="all">Todos</option></select></label></div></div>
