...
const RESOLUTION_TYPES = [
  { value: 'physical_item_found', label: 'Item físico localizado' },
  { value: 'inventory_count_corrected', label: 'Contagem de estoque corrigida' },
  { value: 'loss_or_waste_registered', label: 'Perda/quebra registrada' },
  { value: 'registration_error', label: 'Erro de cadastro ou local' },
...
export default function StockDiscrepanciesPage() {
  ...
  const [nextStatus, setNextStatus] = useState<'under_review' | 'waiting_stock_count' | 'resolved' | 'cancelled'>('under_review');
  ...
  const filteredOccurrences = useMemo(() => {
    ...
    return occurrences.filter((occurrence) => {
      const productNames = occurrence.items
  ...
  function openTreatment(occurrence: StockDiscrepancyOccurrence) {
    setSelected(occurrence);
    setNextStatus(
      occurrence.status === 'waiting_stock_count'
        ? 'waiting_stock_count'
        : 'under_review'
    );
  ...
  const openCount = occurrences.filter((item) => item.status === 'open').length;
  const treatmentCount = occurrences.filter(
    (item) => item.status === 'under_review' || item.status === 'waiting_stock_count'
  ).length;
      ...
      <div className="space-y-5">
        <header className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-3xl text-sm text-gray-500 dark:text-gray-400">
              Vendas concluídas sem saldo suficiente permanecem nesta fila até a conferência física e o registro da causa.
            </p>
          </div>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-2xl border border-amber-100 bg-amber-50 p-4 dark:border-amber-900/50 dark:bg-amber-950/30">
            ...
            <p className="mt-2 text-2xl font-black text-amber-900 dark:text-amber-100">{openCount}</p>
          </div>
          <div className="rounded-2xl border border-blue-100 bg-blue-50 p-4 dark:border-blue-900/50 dark:bg-blue-950/30">
            ...
            <p className="mt-2 text-2xl font-black text-blue-900 dark:text-blue-100">{treatmentCount}</p>
          </div>
        </div>
            ...
            {filteredOccurrences.map((occurrence) => (
              <article key={occurrence.id} className="rounded-3xl border border-gray-100 bg-white p-5 shadow-xs dark:border-gray-800 dark:bg-gray-900">
                <div className="flex flex-col gap-4 xl:flex-row xl:justify-between">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="inline-flex items-center gap-1 rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-800 dark:bg-amber-950 dark:text-amber-200">
                        {occurrence.status === 'resolved' ? <CheckCircle2 size={13} /> : <AlertTriangle size={13} />}
                  ...
                  {canResolve && (
                    <button type="button" onClick={() => openTreatment(occurrence)} className="h-fit shrink-0 rounded-xl bg-teal-600 px-4 py-2 text-sm font-black text-white hover:bg-teal-700">
                      Tratar divergência
                    </button>
                  )}
                </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/50 sm:items-center sm:p-4">
          <div className="max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl dark:bg-gray-900 sm:rounded-3xl">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Venda {selected.order_code || selected.order_id?.slice(0, 8) || '—'} · {selected.location_name || 'local não identificado'}
            </p>
            <div className="mt-5 space-y-4">
              <label className="block text-sm font-black text-gray-700 dark:text-gray-200">
                Próximo estado
                <select value={nextStatus} onChange={(event) => setNextStatus(event.target.value as typeof nextStatus)} className="mt-2 w-full rounded-xl border border-gray-200 bg-white px-3 py-2 dark:border-gray-700 dark:bg-gray-950 dark:text-white">
                  <option value="under_review">Em análise</option>
                  <option value="waiting_stock_count">Aguardando contagem</option>
              ...
              <label className="block text-sm font-black text-gray-700 dark:text-gray-200">
                ...
                <select value={resolutionType} onChange={(event) => setResolutionType(event.target.value)} className="mt-2 w-full rounded-xl border border-gray-200 bg-white px-3 py-2 dark:border-gray-700 dark:bg-gray-950 dark:text-white">
                  ...
                  {RESOLUTION_TYPES.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
                </select>
              </label>
              <label className="block text-sm font-black text-gray-700 dark:text-gray-200">
                O que foi conferido?
            ...
            <div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
              ...
              <button type="button" onClick={() => void saveTreatment()} disabled={saving} className="rounded-xl bg-teal-600 px-4 py-2 text-sm font-black text-white disabled:opacity-60">
