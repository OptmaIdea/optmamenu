...
export default function FinancialAccountsWorkspacePage() {
  ...
  const compatibleBulkAccounts = useMemo(() => {
    ...
    return activeAccounts.filter((account) => selectedPaymentCodes.every((code) => account.accepted_payment_methods?.includes(code)));
  ...
  const transferDestinations = transferForm
    ? activeAccounts.filter((account) =>
        account.id !== transferForm.sourceAccountId && account.accepted_payment_methods?.includes(transferForm.paymentMethodCode),
      )
    : [];
                  ...
                  {accounts.map((account) => (
                    <div key={account.id} className={`rounded-2xl border p-4 ${account.active ? 'border-gray-100 bg-gray-50 dark:border-gray-800 dark:bg-gray-950/50' : 'border-gray-200 bg-gray-100 opacity-65 dark:border-gray-800 dark:bg-gray-950'}`}>
                      ...
                      {account.payment_breakdown.length > 0 && <div className="mt-4 space-y-2 border-t border-gray-200 pt-3 dark:border-gray-800"><p className="text-[10px] font-black uppercase tracking-widest text-gray-400">Composição do saldo</p>{account.payment_breakdown.map((item) => {
                        const destinations = activeAccounts.filter((candidate) => candidate.id !== account.id && candidate.accepted_payment_methods?.includes(item.payment_method_code));
            ...
            {activeTab === 'unallocated' && (
              <section className="rounded-2xl border border-amber-200 bg-amber-50/50 p-5 dark:border-amber-900/50 dark:bg-amber-950/10">
                ...
                <div className="rounded-2xl border border-amber-200 bg-white p-4 dark:border-amber-900/40 dark:bg-gray-900">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => setPaymentFilter('all')} className={`rounded-lg border px-3 py-2 text-xs font-black ${paymentFilter === 'all' ? 'border-teal-500 bg-teal-50 text-teal-700' : 'text-gray-600'}`}>Todos ({periodFilteredUnallocated.length})</button>{unallocatedByPayment.map(([code, count]) => <button key={code} type="button" onClick={() => setPaymentFilter(code)} className={`rounded-lg border px-3 py-2 text-xs font-black ${paymentFilter === code ? 'border-teal-500 bg-teal-50 text-teal-700' : 'border-amber-200 bg-amber-50 text-amber-800'}`}>{paymentName(code)} ({count})</button>)}</div>
                ...
                <div className="mt-4 space-y-3">{visibleUnallocated.length === 0 ? <div className="rounded-xl bg-white p-6 text-center text-sm font-bold text-gray-500 dark:bg-gray-900">Nenhum lançamento para os filtros atuais.</div> : visibleUnallocated.map((entry) => {
                  ...
                  return <div key={entry.id} className={`rounded-xl border bg-white p-4 shadow-sm dark:bg-gray-900 ${selectedRows[entry.id] ? 'border-amber-500 ring-1 ring-amber-300' : 'border-amber-100 dark:border-amber-900/40'}`}>
                    ...
                    {canManage && <div className="mt-4 grid gap-2 xl:grid-cols-[1fr_1fr_1fr_auto]"><select value={effectiveCode} onChange={(event) => { setEntryPaymentCodes((current) => ({ ...current, [entry.id]: event.target.value })); setSelectedAccounts((current) => ({ ...current, [entry.id]: '' })); }} className="rounded-xl border px-3 py-2 text-sm font-bold dark:border-gray-700 dark:bg-gray-950"><option value="pending">Pendente</option>{receiptPaymentMethods.map((method) => <option key={method.code} value={method.code}>{method.name}</option>)}</select><select value={selectedAccounts[entry.id] || ''} onChange={(event) => setSelectedAccounts((current) => ({ ...current, [entry.id]: event.target.value }))} disabled={effectiveCode === 'pending'} className="rounded-xl border px-3 py-2 text-sm font-bold disabled:opacity-50 dark:border-gray-700 dark:bg-gray-950"><option value="">{effectiveCode === 'pending' ? 'Troque a forma para escolher a conta' : 'Selecione a conta compatível'}</option>{compatibleAccounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select><input value={reasons[entry.id] || ''} onChange={(event) => setReasons((current) => ({ ...current, [entry.id]: event.target.value }))} placeholder="Motivo/observação (opcional)" className="rounded-xl border px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-950" /><button type="button" onClick={() => void saveUnallocatedEntry(entry)} disabled={workingId === entry.id || (!methodChanged && !accountSelected) || effectiveCode === 'pending'} className="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-600 px-4 py-2 text-sm font-black text-white disabled:opacity-40">{workingId === entry.id ? <Loader2 size={15} className="animate-spin" /> : <Save size={15} />}{methodChanged ? (accountSelected ? 'Salvar e distribuir' : 'Salvar forma') : 'Distribuir'}</button></div>}
                  </div>;
                })}</div>
              </section>
            )}
