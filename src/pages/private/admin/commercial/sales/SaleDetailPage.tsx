...
export default function SaleDetailPage() {
  ...
  const partialEstimate = useMemo(() => {
    if (!detail || !adjustmentForm || adjustmentForm.type !== 'partial_return') return 0;
    return detail.items.reduce((total, item) => {
      const quantity = adjustmentForm.quantities[item.id] || 0;
      const unitNet = item.quantity > 0 ? item.line_total / item.quantity : 0;
      return total + unitNet * quantity;
    }, 0);
  }, [adjustmentForm, detail]);
  ...
  if (!detail) {
    return <PageContainer title="Detalhe da venda" withoutHeader><div className="rounded-2xl border border-red-200 bg-red-50 p-8 text-center dark:border-red-900 dark:bg-red-950/30"><p className="font-black text-red-800 dark:text-red-200">Venda indisponível.</p><button type="button" onClick={() => navigate(-1)} className="mt-4 inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2 text-sm font-black text-red-700 dark:bg-slate-900 dark:text-red-300"><ArrowLeft size={16} />Voltar</button></div></PageContainer>;
  }
  ...
  function openAdjustment(type: SaleAdjustmentType) {
    const preferredAccount = financialAccounts.find((account) => account.id === originalAccountId)?.id || financialAccounts[0]?.id || '';
    setAdjustmentForm({
      type,
      reasonCode: type === 'full_cancellation' ? 'customer_withdrew' : 'customer_return',
      notes: '',
      refundAccountId: preferredAccount,
      quantities: {},
    });
  }
  ...
  async function submitAdjustment() {
    ...
    try {
      ...
      toast.success(`${adjustmentForm.type === 'full_cancellation' ? 'Cancelamento/estorno' : 'Devolução parcial'} registrado: ${currency.format(result.refundAmount)}.`);
    ...
    return (
      <PageContainer title={focusTitle} withoutHeader>
        <div className="space-y-5 text-slate-900 dark:text-slate-100">
          <div>
            <button type="button" onClick={goBack} className="mb-3 inline-flex items-center gap-2 text-sm font-black text-teal-700 dark:text-teal-300"><ArrowLeft size={16} />Voltar</button>
    ...
    <PageContainer title="Detalhe da venda" withoutHeader>
      <div className="space-y-5 text-slate-900 dark:text-slate-100">
        ...
        {detail.adjustments.length > 0 && <Section title="Cancelamentos e devoluções" subtitle="Histórico auditável das reversões feitas depois da conclusão da venda." icon={<RotateCcw size={18} />}>
          <div className="space-y-2">{detail.adjustments.map((adjustment) => <div key={adjustment.id} className="rounded-xl border border-slate-100 p-4 dark:border-slate-800"><div className="flex flex-col gap-2 sm:flex-row sm:justify-between"><div><p className="font-black">{adjustmentTypeLabels[adjustment.adjustment_type]}</p><p className="mt-1 text-xs font-semibold text-slate-500 dark:text-slate-400">{formatDate(adjustment.created_at)} · {reasonLabels[adjustment.reason_code]}</p><p className="mt-1 text-sm font-semibold">{adjustment.reason_notes}</p></div><p className="text-xl font-black text-red-700 dark:text-red-300">-{currency.format(adjustment.refund_amount)}</p></div>{adjustment.items.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{adjustment.items.map((item) => <span key={item.id} className="rounded-lg bg-slate-50 px-2.5 py-1.5 text-xs font-bold dark:bg-slate-950">{item.product_name} · {item.quantity} un. · estoque retornado {item.stock_returned_quantity}</span>)}</div>}</div>)}</div>
        </Section>}
      ...
      {adjustmentForm && (
        <div className="fixed inset-0 z-[120] flex items-end justify-center bg-black/60 p-0 backdrop-blur-sm sm:items-center sm:p-4">
          <div className="max-h-[94vh] w-full max-w-2xl overflow-y-auto rounded-t-3xl bg-white shadow-2xl dark:bg-slate-900 sm:rounded-3xl">
            <div className="flex items-start justify-between border-b border-slate-200 p-5 dark:border-slate-800"><div><h2 className="text-xl font-black text-slate-900 dark:text-white">{adjustmentTypeLabels[adjustmentForm.type]}</h2><p className="mt-1 text-xs font-semibold text-slate-500 dark:text-slate-400">A venda original não é apagada. O sistema registra a reversão no estoque, no Livro Diário e na conta financeira.</p></div><button type="button" onClick={() => setAdjustmentForm(null)} className="rounded-xl border border-slate-200 p-2 text-slate-500 dark:border-slate-700"><X size={16} /></button></div>
            <div className="space-y-4 p-5">
              ...
              <label className="block"><span className="text-xs font-black uppercase tracking-widest text-slate-400">Conta de onde sairá o estorno</span><select value={adjustmentForm.refundAccountId} onChange={(event) => setAdjustmentForm((current) => current ? { ...current, refundAccountId: event.target.value } : current)} className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 font-bold dark:border-slate-700 dark:bg-slate-950"><option value="">Selecione uma conta ativa</option>{financialAccounts.map((account) => <option key={account.id} value={account.id}>{account.name} · saldo {currency.format(account.balance)}</option>)}</select>{financialAccounts.length === 0 && <p className="mt-1 text-xs font-bold text-red-600 dark:text-red-300">Nenhuma conta financeira ativa disponível. Configure uma conta antes de estornar.</p>}</label>

              {adjustmentForm.type === 'partial_return' ? <div><p className="text-xs font-black uppercase tracking-widest text-slate-400">Itens devolvidos</p><div className="mt-2 space-y-2">{items.map((item) => { const remaining = Math.max(item.quantity - (returnedByItem[item.id] || 0), 0); const unitNet = item.quantity > 0 ? item.line_total / item.quantity : 0; return <div key={item.id} className={`grid gap-3 rounded-xl border p-3 dark:border-slate-700 sm:grid-cols-[minmax(180px,1fr)_120px_120px] sm:items-center ${remaining === 0 ? 'opacity-50' : ''}`}><div><p className="font-black">{item.product_name}</p><p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Disponível para devolução: {remaining}/{item.quantity} · {currency.format(unitNet)} por un.</p></div><label className="text-xs font-black text-slate-500">Quantidade<input type="number" min={0} max={remaining} step={1} disabled={remaining === 0} value={adjustmentForm.quantities[item.id] || ''} onChange={(event) => { const parsed = Math.max(0, Math.min(remaining, Number.parseInt(event.target.value || '0', 10) || 0)); setAdjustmentForm((current) => current ? { ...current, quantities: { ...current.quantities, [item.id]: parsed } } : current); }} className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 dark:border-slate-700 dark:bg-slate-950" /></label><div className="sm:text-right"><p className="text-xs font-bold text-slate-400">Estorno estimado</p><p className="font-black text-red-700 dark:text-red-300">{currency.format(unitNet * (adjustmentForm.quantities[item.id] || 0))}</p></div></div>; })}</div><div className="mt-3 rounded-xl bg-amber-50 p-3 text-sm font-bold text-amber-900 dark:bg-amber-950/30 dark:text-amber-200">Estorno parcial estimado: <strong>{currency.format(partialEstimate)}</strong>. O backend recalcula o valor autorizado com o snapshot da venda.</div></div> : <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-900 dark:border-red-900 dark:bg-red-950/30 dark:text-red-200"><p className="font-black">Estorno total restante: {currency.format(detail.remainingRefundable)}</p><p className="mt-1 font-semibold">O estoque retorna somente para quantidades que tiveram baixa física vinculada. Divergências sem baixa não geram estoque fictício.</p></div>}
            </div>
            <div className="flex flex-col-reverse gap-2 border-t border-slate-200 p-5 dark:border-slate-800 sm:flex-row sm:justify-end"><button type="button" onClick={() => setAdjustmentForm(null)} className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-black dark:border-slate-700">Voltar sem alterar</button><button type="button" onClick={() => void submitAdjustment()} disabled={adjustmentSaving || !adjustmentForm.refundAccountId || financialAccounts.length === 0} className={`inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2 text-sm font-black text-white disabled:opacity-50 ${adjustmentForm.type === 'full_cancellation' ? 'bg-red-600' : 'bg-amber-600'}`}>{adjustmentSaving ? <Loader2 size={15} className="animate-spin" /> : adjustmentForm.type === 'full_cancellation' ? <RotateCcw size={15} /> : <Undo2 size={15} />}{adjustmentForm.type === 'full_cancellation' ? 'Confirmar cancelamento e estorno' : 'Confirmar devolução parcial'}</button></div>
          </div>
        </div>
      )}
    </PageContainer>
