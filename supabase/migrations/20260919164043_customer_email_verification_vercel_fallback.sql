...
begin
  ...
  where c.store_id=v_store_id and c.customer_id=v_customer_id
  ...
  if v_recent_count>=1 or v_hourly_count>=5 then
    return jsonb_build_object('ok',false,'error','rate_limited');
  end if;

  select s.name,s.slug into v_store
...
create or replace function public.mark_customer_self_email_verification_delivery_safe(
  p_challenge_id uuid,
...
begin
  ...
  update public.customer_email_verification_challenges
  set provider_message_id=nullif(trim(coalesce(p_provider_message_id,'')),''),
      used_at=case when coalesce(p_failed,false) then coalesce(used_at,clock_timestamp()) else used_at end,
      metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object(
        'provider',nullif(trim(coalesce(p_provider,'')),''),
        'delivery_failed',coalesce(p_failed,false),
...
revoke all on function public.mark_customer_self_email_verification_delivery_safe(uuid,text,text,boolean) from public,anon;
...
grant execute on function public.mark_customer_self_email_verification_delivery_safe(uuid,text,text,boolean) to authenticated,service_role;
