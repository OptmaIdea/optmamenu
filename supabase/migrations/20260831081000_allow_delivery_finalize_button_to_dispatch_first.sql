-- Enquanto a UI de pedidos ainda tem um botão genérico de finalização em delivery,
-- esta proteção transforma o primeiro clique em despacho: baixa estoque e envia para entrega,
-- sem marcar pagamento como pago antes do retorno da entrega.
...
begin
  select id,store_id,order_code,status,fulfillment_type into v_order from public.orders where id=p_order_id;
  ...
  if coalesce(v_order.fulfillment_type, 'pickup') = 'delivery' and v_order.status::text <> 'out_for_delivery' then
    v_result := public.admin_dispatch_public_order_safe_internal_0d(p_order_id);
    return v_result || jsonb_build_object(
      'payment_status', 'pending',
      'payment_method_code', v_method_code,
      'dispatch_only', true,
      'message', 'Pedido enviado para entrega. Confirme o recebimento no retorno da entrega.'
    );
  end if;
