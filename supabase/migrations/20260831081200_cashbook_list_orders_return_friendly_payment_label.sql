  ...
  select coalesce(
    jsonb_agg(
      (
        to_jsonb(q)
        || jsonb_build_object(
          'payment_method_code', case when q.source = 'order' then null else q.payment_method_code end,
          'payment_method', coalesce(q.payment_method, q.friendly_payment_label),
