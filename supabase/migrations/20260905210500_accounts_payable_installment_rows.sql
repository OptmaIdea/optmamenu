-- Contas a Pagar: a lista principal passa a representar cada parcela individualmente.
-- O titulo contabil continua sendo unico por compra; esta migration altera apenas o read model.
...
begin
  ...
  if not public.user_can_access_store(auth.uid(), p_store_id) then
    raise exception 'Sem acesso à loja informada.';
  end if;

  if not public.user_can(auth.uid(), p_store_id, 'accounts_payable.view') then
    ...
    with installment_rows as (
      select
        ...
        case
          ...
          else count(i.id) over (partition by p.id)
      ...
      left join public.accounts_payable_installments i
        on i.accounts_payable_id = p.id
...
begin
  ...
  if not public.user_can_access_store(auth.uid(), v_p.store_id) then
    raise exception 'Sem acesso à loja da conta a pagar.';
  end if;

  if not public.user_can(auth.uid(), v_p.store_id, 'accounts_payable.view') then
    raise exception 'Sem permissão para consultar contas a pagar.';
  end if;
  ...
  return jsonb_build_object(
    ...
    'installments', coalesce((
      ...
      from public.accounts_payable_installments i
      where i.accounts_payable_id = v_p.id
    ), '[]'::jsonb),
    'adjustments', coalesce((
      select jsonb_agg(to_jsonb(a) order by a.created_at desc)
      from public.accounts_payable_adjustments a
      where a.accounts_payable_id = v_p.id
    ), '[]'::jsonb),
    'payments', coalesce((
      select jsonb_agg(to_jsonb(pay) order by pay.paid_at desc, pay.created_at desc)
      from public.accounts_payable_payments pay
      where pay.accounts_payable_id = v_p.id
    ), '[]'::jsonb),
    'events', coalesce((
      select jsonb_agg(to_jsonb(e) order by e.created_at desc)
      from public.accounts_payable_events e
      where e.accounts_payable_id = v_p.id
    ), '[]'::jsonb)
  );
end;
$$;
