-- POS_9 — Venda direta — desconto manual e metadados comerciais
--
-- Objetivo:
-- Evoluir a RPC de venda direta sem criar schema novo:
-- - preservar desconto manual por item em order_items.discount;
-- - aceitar metadados de precificação enviados por item;
-- - registrar subtotal bruto, desconto total e total final em orders.commercial_metadata;
-- - registrar preço original/aplicado e regra aplicada em order_items.commercial_metadata.
--
-- Observação:
-- Nesta etapa, a UI/service calculam a primeira camada de price_rules por quantidade.
-- O backend continua validando produto, estoque e permissões, e persiste os metadados.
