-- POS_9 — Correção da RPC de venda direta
--
-- Motivo:
-- A versão inicial usava `v_payment_method record` e, no fluxo `pending`, tentava
-- atribuir campos diretamente antes do record receber uma linha.
-- Isso gerava: record "v_payment_method" is not assigned yet.
--
-- Correção:
-- Recriar a função usando variáveis escalares para pagamento.
