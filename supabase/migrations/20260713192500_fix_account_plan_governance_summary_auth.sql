-- POS_9 — Ajuste do resumo de governança para diagnóstico via SQL Editor
--
-- Contexto:
-- O SQL Editor do Supabase executa consultas sem um JWT de usuário final; por isso auth.uid()
-- fica NULL mesmo quando o operador está logado no painel. Esta RPC é concedida apenas ao
-- papel authenticated, então não precisa retornar "Usuário não autenticado" quando executada
-- como diagnóstico administrativo no SQL Editor.
